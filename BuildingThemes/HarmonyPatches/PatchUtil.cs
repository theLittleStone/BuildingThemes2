...
namespace BuildingThemes.HarmonyPatches
{
    public static class PatchUtil
    {
            ...
            MethodDefinition transpiler = null)
        {
            ...
            try
            {
                Debugger.LogFormat("Patching method {0}.{1}", original.Type.FullName, original.MethodName);
            ...
            {
                Debugger.LogError($"Failed to patch method {original.Type.FullName}.{original.MethodName}");
        ...
        {
            Debugger.LogFormat("Unpatching method {0}.{1}", original.Type.FullName, original.MethodName);
        ...
        private static MethodInfo GetOriginal(MethodDefinition original)
        {
            ...
            {
                throw new Exception(
                    $"Building Themes: Failed to find original method {original.Type.FullName}.{original.MethodName}");
            }
        ...
        private static MethodInfo GetPatch(MethodDefinition patch)
        {
            ...
            if (methodInfo == null)
            {
                throw new Exception($"Building Themes: Failed to find patch method {patch.Type.FullName}.{patch.MethodName}");
            }
