        ...
        private void SetupControls()
        {
            ...
            m_policyCheckBox.eventCheckChanged += delegate(UIComponent component, bool isChecked)
            {
                lock (component)
                {
                    ...
                    if (isChecked)
                    {
                        Debugger.LogFormat("[UserAction] district {0} — enabled theme '{1}'.", districtId, m_theme != null ? m_theme.name : "null");
                        BuildingThemesManager.instance.EnableTheme(districtId, m_theme);
                    }
                    else
                    {
                        Debugger.LogFormat("[UserAction] district {0} — disabled theme '{1}'.", districtId, m_theme != null ? m_theme.name : "null");
